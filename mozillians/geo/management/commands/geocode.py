...
def lookup_location(city, region, country):
    map_id = MAPBOX_MAP_ID

    name = ' '.join([x for x in (city, region, country) if x]).replace(' ', '+')
    url = 'http://api.tiles.mapbox.com/v3/%s/geocode/%s.json' % (map_id, name)
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # RESET - FIXME!
        # UserProfile.objects.filter(geo_coded=True).update(geo_coded=False,
        #                            geo_country=None, geo_region=None, geo_city=None)
        # Geocoding.objects.all().delete()
        ...
        # sys.exit(1)
        #
        # # Find the bounds on the cities we have already without bounds
        # print("Cities without bounds: %d" % City.objects.filter(w=None).count())
        # for city in City.objects.filter(w=None):
        #     name = ' '.join([x.name for x in (city, city.region, city.country) if x])
        #     print(name)
        ...
        #     results = massage_results(city.country.code, results)
            # That will have added the bounds to our city if it didn't have it already

        # # Save data from records we've already geocoded
        # geocoded = UserProfile.objects.exclude(geo_region=None, geo_country=None, geo_city=None)
        # for profile in geocoded:
        #     Geocoding.remember(profile)
        #
        ...
        for profile in qset.order_by('country', 'region', 'city'):
            ...
            name = ' '.join([x for x in (city, region, country) if x])
            print(repr(name))
            encoded_name = name.encode('utf8')
            quoted_name = urllib.quote_plus(encoded_name, '')
            url = 'http://api.tiles.mapbox.com/v3/%s/geocode/%s.json' % (map_id, quoted_name)
